...
<script type="text/javascript">
	$(function(){
		...
		$(".coverSave").click(function(){
			var dietDate = $("#dietDate").val();
			var formData = new FormData($("updateDietForm"));
			formData.append("dietId", $("#dietId").val());
			...
			$.ajax({
				url:"${pageContext.request.contextPath }/updateBabyDietById",
				type:"post",
				dataType:"text",
				data:formData,
				...
				success:function(data){
					console.log(data)
					if(data=="true"){
						window.location.href="${pageContext.request.contextPath }/listBabyDietByTerm?dietDate="+dietDate;
						/* 若录入食谱失败，给出提示信息 */
						//new TipBox({type:'tip',str:'宝宝食谱修改成功!',clickDomCancel:true,setTime:10000500,hasBtn:true});  
					}else{
						/* 若录入食谱失败，给出提示信息 */
						new TipBox({type:'tip',str:'宝宝食谱修改失败!',clickDomCancel:true,setTime:10000500,hasBtn:true});  
					}
				}   
			});
		});
...
<body>
	<div >
		...
		<div class="title">
			...
			<span style="margin-left:260px;margin-right:260px;">修改宝宝食谱</span>
				...
				<table class="table table-bordered" style="font-size:40px">
					<tbody>
						<tr>
							...
							<td colspan="2">
								<input type="text" value="${diet.diet_date}" class="form-control" id="dietDate" name="dietDate"readonly/>
							</td>
							<input value="${diet.diet_id}" id="dietId" name="dietId" hidden="hidden">
						</tr>
						<tr>
							<th><span>早饭名称</span></th>
							<td colspan="2">
								<input value="${diet.breakfast}" id="breakfast" name="breakfast" class="form-control">
							</td>
						</tr>
						<tr>
							<th><span>午饭名称</span></th>
							<td colspan="2">
								<input value="${diet.lunch}" id="lunch" name="lunch" class="form-control">
							</td>
						</tr>
						<tr>
							<th><span>晚饭名称</span></th>
							<td colspan="2">
								<input value="${diet.dinner}" id="dinner" name="dinner" class="form-control">
							</td>
						</tr>
						<tr>
							<th><span>早饭图片</span></th>
							<td >
								<img src="${diet.breakfast_img}" id="bImg" name="bImg">
							</td>
							<td>
								<input type="file" id="breakfastPath" name="breakfastPath">
						...
						<tr>
							...
							<td >
								<img src="${diet.lunch_img}" id="lImg" name="lImg">
							</td>
							<td>
								<input type="file" id="lunchPath" name="lunchPath">
						...
						<tr>
							...
							<td >
								<img src="${diet.dinner_img}" id="dImg" name="dImg">
							</td>
							<td>
