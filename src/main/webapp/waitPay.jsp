...
<script type="text/javascript">
	$(function(){
		...
		$("#prevPage").click(function() {
			...
			window.location.href = "${pageContext.request.contextPath }/listWaitPayBabyInfo?curPage="+curPage+"&pageSize="+pageSize;
		});
	...
	function notice(mobile){
		$.ajax({
			url:'${pageContext.request.contextPath }/noticePayTuition',
			type:'get',
			dataType:'json',
			data:{
				'mobile':mobile,
			...
			success:function(data){
				if(data=="OK"){
					new TipBox({type:'success',str:'短信发送成功!',hasBtn:true});
				}else{
					new TipBox({type:'error',str:'短信发送失败!',hasBtn:true});  
				} 
			}
		});
	}
	...
	<div>
		...
		<div class="content">
			...
			<ul class="nav nav-pills" role="tablist">
	  			...
	  			<li class="active"><a href="/babyassistant/listWaitPayBabyInfo">待付款</a></li>
			...
			<table class="table table-bordered table-hover" style="font-size:40px" id="classAttendance">
				...
				<tbody>
					<c:forEach items="${pv.datas}" var="waitPayClassInfo">
						<tr>
							<td>${waitPayClassInfo.grade_name}</td>
							<td>${waitPayClassInfo.baby_name}</td>
							<td>${waitPayClassInfo.username}</td>
							<td>
								<input type="button" class="btn btn-info" value="短信通知" style="font-size:35px;" onclick="notice('${waitPayClassInfo.username}')">
							</td>
						 </tr>
					</c:forEach>
					<tr>
